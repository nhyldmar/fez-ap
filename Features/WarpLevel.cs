...
{
    internal class WarpLevel : IFezapCommand
    {
		...
		public bool Execute(string[] args)
        {
			...
			if(args.Length == 0)
            {
				FezapConsole.Print("List of available levels:");
				FezapConsole.Print(String.Join(", ", LevelList));
				return true;
			}
			...
			{
				FezapConsole.Print($"Couldn't find level with name '{levelName}'", FezapConsole.OutputType.Warning);
				return false;
			}

			Warp(levelName);

			FezapConsole.Print($"Warping into '{levelName}'...");

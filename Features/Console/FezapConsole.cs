...
{
    public class FezapConsole : IFezapFeature
    {
        ...
        public class AutocompletionManager
        {
            ...
            public void Refresh(ParsedCommandSequence commandSequence)
            {
                ...
                var matchingVariables = FezapVariable.DefinedList.Where(var => var.Name.StartsWith(command[0], StringComparison.OrdinalIgnoreCase));
        ...
        {
            public List<IFezapCommand> Commands { get; private set; }
            ...
            public CommandHandler()
            {
                ...
                Commands = new List<IFezapCommand>();
                foreach (Type type in Assembly.GetExecutingAssembly().GetTypes()
                .Where(t => t.IsClass && typeof(IFezapCommand).IsAssignableFrom(t)))
                {
                    IFezapCommand command;
                    // command object could've been already creates as a feature - look for it first
                    IFezapFeature feature = Fezap.GetFeature(type);
                    if(feature != null)
                    {
                        command = (IFezapCommand)feature;
                    }
                    else
                    {
                        command = (IFezapCommand)Activator.CreateInstance(type);
            ...
            public void ExecuteCommand(string commandString)
            {
                ...
                foreach (var command in commands)
                {
                    ...
                    var matchingVariables = FezapVariable.DefinedList.Where(var => var.Name.Equals(command.Command, StringComparison.OrdinalIgnoreCase));

                    if (matchingCommands.Count() == 0 && matchingVariables.Count() == 0)
                    {
                        FezapConsole.Print($"Unknown command: {mainCommand}", OutputType.Error);
                        continue;
                    }
                    ...
                    else if(matchingVariables.Count() > 0)
                    {
                        ...
                        {
                            FezapConsole.Print($"{variable.Name} = {variable.ValueString}");
                        }
        ...
        public static FezapConsole Instance { get; private set; }

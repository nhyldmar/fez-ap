...
{
    internal class BlackHoleManip : IFezapCommand
    {
        ...
        public bool Execute(string[] args)
        {
            ...
            switch (args[0])
            {
                case "on":
                    EnableBlackHoles();
                    FezapConsole.Print($"Black holes have been enabled.");
                    break;
                case "off":
                    DisableBlackHoles();
                    FezapConsole.Print($"Black holes have been disabled.");
                    break;
                case "lock":
                    Locked = true;
                    cachedState = AreBlackHolesEnabled();
                    FezapConsole.Print($"Black holes have been locked in {(cachedState ? "enabled" : "disabled")} state.");
                    break;
                case "unlock":
                    Locked = false;
                    FezapConsole.Print($"Black holes state has been unlocked.");
                    break;
                default:
                    FezapConsole.Print($"Unknown parameter: '{args[0]}'", FezapConsole.OutputType.Warning);
                    return false;
            }

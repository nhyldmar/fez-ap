...
{
    internal class MapItemSet : IFezapCommand
    {
        ...
        public bool Execute(string[] args)
        {
            ...
            if (args.Length != 2 || args[0] == "list")
            {
                ...
                {
                    FezapConsole.Print($"List of maps in your inventory:");
                    FezapConsole.Print($"{String.Join(", ", GameState.SaveData.Maps.Select(s=>s.ToLower()))}");
                }
                else
                {
                    FezapConsole.Print($"List of available maps:");
                    FezapConsole.Print($"{String.Join(", ", mapList.Select(s => s.ToLower()))}");
                }
                return true;
            }
            ...
            if (!mapList.Contains(args[1].ToUpper()))
            {
                if (args[0] == "remove" && args[1] == "all")
                {
                    ...
                    FezapConsole.Print($"Removed all maps from player");
                    return true;
                }
                FezapConsole.Print($"Map with given name does not exist.", FezapConsole.OutputType.Warning);
                return false;
            }
            ...
            if(args[0] == "remove")
            {
                ...
                {
                    FezapConsole.Print("You don't have this map in your inventory.", FezapConsole.OutputType.Warning);
                    return false;
                }
                GameState.SaveData.Maps.Remove(mapName);
                FezapConsole.Print($"Map \"{mapName}\" has been removed from your inventory!");
            }
            else
            {
                if (GameState.SaveData.Maps.Contains(mapName))
                {
                    FezapConsole.Print("You already have this map in your inventory.", FezapConsole.OutputType.Warning);
                    return false;
                }
                GameState.SaveData.Maps.Add(mapName);
                FezapConsole.Print($"Map \"{mapName}\" has been added to your inventory!");
            }
